...
if __name__ == '__main__':
    ...
    develop_CLI_SSH = False
    develop_CLI_SQL = False
    # Main program
    ...
    if log == 0:
        ...
        cur_log.execute('''insert into DevelopSet Values(1,0)''')  # Web.py
        cur_log.execute('''insert into DevelopSet Values(2,0)''')  # celebrate
        cur_log.execute('''insert into DevelopSet Values(3,0)''')  # Client(SSH)
        cur_log.execute('''insert into DevelopSet Values(4,0)''')  # Client(SQL)
        conn_log.commit()
    ...
    while True:
        ...
        for developset_in in cur_log.fetchall():
            ...
            elif developset_in[0] == 3 and developset_in[1] == 1:
                develop_CLI_SSH = True
            elif developset_in[0] == 4 and developset_in[1] == 1:
                develop_CLI_SQL = True
        conn_log.close()
        # Main Program
        os.system("clear")
        ...
        if main_set == 0:
            ...
            print("==========================================")
            print("0: Back")
            print("1: Website version background management system: " + str(develop_web))
            print("2: Special date celebration (will take up a small amount of resources): " + str(develop_celebrate))
            print("3: Local visual control system (SSH): " + str(develop_CLI_SSH))
            print("4: Local visual control system (SQL): " + str(develop_CLI_SQL))
            develop_set_input = int(input())
            if develop_set_input == 0:
                continue
            elif develop_set_input == 1:
                if webpy_import:
                    conn_log = sqlite3.connect("log.db")
                    cur_log = conn_log.cursor()
                    ...
                    print("==========================================")
                    print("The experimental feature is currently being used. This feature has not been fully tested and may cause unpredictable consequences.")
                    time.sleep(3)
                    continue
                else:
                    os.system("clear")
                    ...
                    print("Github@underthestars-zhy/PyBlog")
                    print("==========================================")
                    print(">>> pip install web.py")
                    sys.exit()
            elif develop_set_input == 2:
                conn_log = sqlite3.connect("log.db")
                cur_log = conn_log.cursor()
                ...
                time.sleep(3)
                continue
            elif develop_set_input == 3:
                conn_log = sqlite3.connect("log.db")
                cur_log = conn_log.cursor()
                cur_log.execute('''update DevelopSet set tf = 1 where id=3''')
                conn_log.commit()
                conn_log.close()
                os.system("clear")
                print("Develop Set SUCCESS")
                ...
                time.sleep(3)
                continue
            elif develop_set_input == 4:
                conn_log = sqlite3.connect("log.db")
                cur_log = conn_log.cursor()
                cur_log.execute('''update DevelopSet set tf = 1 where id=4''')
                conn_log.commit()
                conn_log.close()
                os.system("clear")
                print("Develop Set SUCCESS")
